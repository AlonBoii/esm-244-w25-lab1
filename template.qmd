...
# janitor:: is a lazy load. You can use a function from a package without installing it package:: function
```
...
Which streets/zip codes have more tree density?

Focus: How many counts on each observation by legal_status? Focus on the highest trees

```{r}
sfo_trees_df |>
  group_by(legal_status) |>
  summarize(tree_count = n()) |>
  slice_max(tree_count, n = 5)|> # what you are slicing based on, and just taking top 5 observations
  ggplot() +
...
```{r}
# == is exactly = to and %in% give you a list of things to look at, and from that list tell me if its there
...
# str_detect = look at a specific column, and in that column anytime you observe "Oak" it'll keep track of it
# "|" is OR
# ifelse() create a logical test, if it exists or doesn't, if it's true what should be the yes and what no
```
...
# separate() separates one column into new columns
```
